...
class ModelLoader:
    ...
    def _validate_env(self):
        ...
        """
        required_vars=["SERPAPI_API_KEY","TAVILY_API_KEY","OPENAI_API_KEY","GROQ_API_KEY","GOOGLE_API_KEY"]
        self.api_keys={key:os.getenv(key) for key in required_vars}
        missing = [k for k, v in self.api_keys.items() if not v]
        if missing:
            #log.error("Missing environment variables", missing_vars=missing)
            raise DocumentPortalException("Missing environment variables", context={"missing_vars":missing})
        log.info("Environment variables validated", available_keys=[k for k in self.api_keys if self.api_keys[k]])
        
    def load_embeddings(self):
        """
        Load and return the embedding model. v
        """
        try:
